...
from bytecode import Bytecode  # type: ignore[import-not-found]
from spasm import Assembly  # type: ignore[import-not-found]
...
def _inject_hook(code: Bytecode, hook: HookType, lineno: int, arg: Any) -> None:
    ...
    if not locs:
        raise InvalidLineError("Line %d does not exist or is either blank or a comment" % lineno)
    ...
    for i in locs:
        code[i:i] = INJECTION_ASSEMBLY.bind({"hook": hook, "arg": arg}, lineno=lineno)
...
def _eject_hook(code: Bytecode, hook: HookType, line: int, arg: Any) -> None:
    ...
    if not locs:
        raise InvalidLineError("Line %d does not contain a hook" % line)
...
def inject_hook(code: CodeType, hook: HookType, line: int, arg: Any) -> CodeType:
    """Inject a hook into a code object.
    ...
    return abstract_code.to_code()


def eject_hook(code: CodeType, hook: HookType, line: int, arg: Any) -> CodeType:
